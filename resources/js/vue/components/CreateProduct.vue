<template>
  <v-app id="inspire">
    ...
    <v-main>
      <v-container
        class="align-center flex-column centerTo justify-center d-flex"
      >
        <h2 class="align-self-start mb-5">Product</h2>
        <v-alert class="w-75 align-self-start" type="success" v-if="success"
        ...
        <v-form class="w-100" ref="form" v-model="valid" lazy-validation>
          ...
          >
            ...
            <v-date-picker v-model="form.manufactured_date" no-title scrollable>
              <v-spacer></v-spacer>
          ...
          >
            ...
            <v-date-picker
              ...
              @change="
                success = false;
                form.errors.clear('expiration_date');
              "
            >
              <v-spacer></v-spacer>
...
export default {
  ...
  mounted() {
    api.getCategories().then((r) => {
      ...
      if (r.data.data[0]) this.form.category_id = r.data.data[0].id;
    });
  },
};
</script>
